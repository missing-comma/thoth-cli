...
const scopeSpecificConfigurations: Record<TestScope, (conf: JestConfig) => JestConfig> = {
	unit: (conf) => {
		conf.testMatch = ['**/*.spec.ts'];
		return conf;
	},
	integration: (conf) => {
		conf.testMatch = ['**/*.test.ts'];
		return conf;
	},
	staged: (conf) => {
...
const config: JestConfig = {
	...
	collectCoverageFrom: ['<rootDir>/src/**/*.ts', '!<rootDir>/src/**/*.d.ts', '!<rootDir>/src/main/**'],
	...
	// setupFiles: ['./src/configs/index.ts'],
	...
	// cacheDirectory: "C:\\Users\\tijuk\\AppData\\Local\\Temp\\jest",
...
function parseConfig(): JestConfig {
	if (testScope) {
		...
		return configMutator ? configMutator(config) : config;
	}
	return config;
}
